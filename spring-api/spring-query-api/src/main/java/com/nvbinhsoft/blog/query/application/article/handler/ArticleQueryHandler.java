...
public class ArticleQueryHandler {
    ...
    public ArticleDetailView handleGetArticle(GetArticleQuery query) {
        return articleViewRepository.findBySlug(query.getSlug())
                .map(article -> new ArticleDetailView(
                        article.getId(),
                        article.getTitle(),
                        article.getSlug(),
                        article.getContent(),
    ...
    public List<ArticleView> handleListArticles(ListArticlesQuery query) {
        return articleViewRepository.findAll(query.getPage(), query.getSize(), query.getCategoryId(), query.getTag())
                .stream()
    ...
    public List<ArticleView> handleGetArticlesByCategory(GetArticlesByCategoryQuery query) {
        return articleViewRepository.findByCategoryId(query.getCategoryId())
                .stream()
                .map(article -> new ArticleView(
                        article.getId(),
                        article.getTitle(),
